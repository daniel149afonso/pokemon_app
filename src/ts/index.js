...
const errorPoke = document.querySelector("#error");
const numberPoke = document.querySelector("#number");
const namePoke = document.querySelector("#name");
const typePoke = document.querySelector("#types");
const weakPoke = document.querySelector("#weakness");
...
async function getWeaknesses(types) {
    try {
        ...
        const allWeaknesses = datas.flatMap(typeData => typeData.damage_relations.double_damage_from.map(t => t.name));
        const uniqueWeaknesses = [...new Set(allWeaknesses)];
        weakPoke.innerHTML = uniqueWeaknesses.map(type => `<span class="badge ${typeColors[type] ?? 'bg-gray-400'}">${type}</span>`).join("");
    }
    catch (error) {
...
function displayInfos(data) {
    ...
    typePoke.innerHTML = types.map(type => `<span class="badge ${typeColors[type] ?? 'bg-gray-400'}">${type}</span>`).join("");
}
